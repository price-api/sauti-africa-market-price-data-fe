...
import { Container, Header, Menu, Table, Popup, Icon, Button } from 'semantic-ui-react';
		...
		<div className={sidenav ? 'side-nav' : 'side-nav hide-nav'}>
			<Menu size='massive' pointing vertical className='side-nav-items'>
				<div className='menu-content'>
					...
					<Menu.Item name='Filter Search Endpoint' onClick={() => scrollToPlay(filter)} />
					<Menu.Item name='Latest Price in All Markets Endpoint' onClick={() => scrollToPlay(latestPrice)} />
					<Menu.Item
						name='Latest Price by Market Endpoint'
						onClick={() => scrollToPlay(lastestMarketPrice)}
					/>
					<Menu.Item name='Latest Price by Date Range Endpoint' onClick={() => scrollToPlay(dateRange)} />
					<Menu.Item name='Errors' onClick={() => scrollToPlay(errorBox)} />
					<Menu.Item name='Get Data Table' onClick={() => scrollToPlay(pivotdocs)} />
				</div>
			...
			<Container fluid className='center-api-column'>
				<section className='articles-examples' ref={api}>
					<article className='left-article'>
						...
						<p>
							Sauti Africa Market Prices API is designed to provide up-to-date daily prices for over 150
							products across more than 100 marketplaces in East Africa. The API has resource-oriented
							URLs, returns JSON-encoded responses and uses standard HTTP response codes, authentication
							and verbs.
						</p>
					</article>
				...
				<section className='articles-examples' ref={quick}>
					<article className='left-article'>
						...
						<p>
							After signing up, every user is assigned a personal API access key, a unique combination of
							letters and digits provided to access any of the API's data endpoints. To authenticate with
							the Sauti Africa Market Prices API, simply pass the API Key as part of the headers.
						</p>
					...
					<article className='right-article'>
						<h3>API Endpoints</h3>
						<p>
							The Sauti Africa Market Prices API offers 5 customizable endpoints, all of which provide
							different kinds of data.
						</p>
						<h5>Base URL: All API requests start out with the following base URL:</h5>
						<Highlight language='javascript'>
							{'https://market-price-api.herokuapp.com//sauti/developer'}
						</Highlight>
						<h5>Available Endpoints</h5>

						<h6>specific list - market, country, source, product</h6>
						<Highlight language='javascript'>
							{'https://market-price-api.herokuapp.com//sauti/developer/lists/'}
						</Highlight>
						...
						<h6>“latest price” - latest prices on a product across all markets</h6>
						<Highlight language='javascript'>
							{'https://market-price-api.herokuapp.com//sauti/developer/product/latestprice/'}
						</Highlight>

						<h6>“latest market price” - latest price on a product in a particular market</h6>
						<Highlight language='javascript'>
							{'https://market-price-api.herokuapp.com//sauti/developer/product/pricebymarket/'}
						</Highlight>

						<h6>“product prices by date range” - prices of a product across a date range</h6>
						<Highlight language='javascript'>
							{'https://market-price-api.herokuapp.com//sauti/developer/product/range/'}
						</Highlight>
					</article>
				</section>
				<section className='articles-examples' ref={refer}>
					<article className='left-article'>
						...
						<h3 style={{ color: 'chartreuse' }} ref={list}>
							GET <span style={{ color: 'black' }}>Specific List: Market, Country, Source, Product</span>
						</h3>
						<p>
							Returns a list based on query parameter of /?list= If incorrect parameter is passed, it will
							default to returning a list of all markets.
							<Popup
						...
						<Highlight language='javascript' className='highlight'>
							{'https://market-price-api.herokuapp.com//sauti/'}
							<br />
						...
						<Table celled>
							...
							<Table.Body>
								<Table.Row>
									...
									<Table.Cell>Enter one of the strings: market, country, source, product</Table.Cell>
								</Table.Row>
							</Table.Body>
						</Table>
					</article>
					<article className='right-article'>
						<img src={listGif} alt='Example lists endpoint' className='gif-examples' />
					</article>
				</section>
				<section className='articles-examples'>
					<article className='left-article'>
						...
						<p>
							Returns array of records via query. Query filters accepted are product,
							product_agg,category, market and country. Additional query parameters for cursor pagination
							include count (optional) and next (optional). Multiple choices in the same filter type can
							be passed too by simply adding same query key with appropriate value. Default is to return
							50 records based on latest dates.
						</p>
						<p>All records are sorted by date descending and id descending.</p>
						<p>Check the example.</p>
						<p>
							This endpoint has cursor pagination built in. Default count of records is 50. Maximum count
							of records is 500 per call. Along with the records, this endpoint returns a next value and a
							topPageValue for ease of use in implementing pagination in applications. On initial calls
							(i.e. when no next value is passed in), a pageCount (total count of pages based on number of
							records and count) will also be returned.{' '}
							<Popup
								trigger={<Icon name='question circle' size='large' />}
								content='You can scroll the URL below horizontally'
								position='right center'
						...
						<Table celled>
							...
							<Table.Body>
								<Table.Row>
									...
									<Table.Cell>
										Product name - 3rd Level - You can retrieve a list of Products from lists
										endpoint
									</Table.Cell>
								</Table.Row>
								<Table.Row>
								...
								<Table.Row>
									...
									<Table.Cell>
										Sets the count for number of records in each call. Max is 500. Default to 50.
									</Table.Cell>
								</Table.Row>
								<Table.Row>
									<Table.Cell>next</Table.Cell>
									<Table.Cell>
										Concatenation of the date and id of the first record on the next page. This
										value is returned to the user on each call. This allows for pagination of
										records.
									</Table.Cell>
								</Table.Row>
							</Table.Body>
					...
					<article className='right-article'>
						<img src={filterGif} alt='Example Filterable search endpoint' className='gif-examples' />
					</article>
				</section>
				<section className='articles-examples'>
					<article className='left-article'>
						...
						<h3 style={{ color: 'chartreuse' }} ref={latestPrice}>
							GET <span style={{ color: 'black' }}>Latest Prices on a Product across all Markets</span>
						</h3>
						<p>Returns all records on given product. Pass the query /?product=</p>
						<Highlight language='javascript'>
							{'https://market-price-api.herokuapp.com//sauti/'}
							<br />
					...
					<article className='right-article'>
						<img src={priceAllGif} alt='Example Filterable search endpoint' className='gif-examples' />
					</article>
				</section>
				<section className='articles-examples'>
					<article className='left-article'>
						...
						<h3 style={{ color: 'chartreuse' }} ref={lastestMarketPrice}>
							GET <span style={{ color: 'black' }}>Latest Price on a product in a particular market</span>
						</h3>
						<p>
				...
				<section className='articles-examples'>
					<article className='left-article'>
						...
						<h3 style={{ color: 'chartreuse' }} ref={dateRange}>
							GET <span style={{ color: 'black' }}>Find prices of a product across a date range</span>
						</h3>
						<p>
							Pass the query paramaters of a product, startDate, endDate, count (optional), and next
							(optional). Sends a response of all available records in that date range separated by cursor
							pagination. This endpoint has cursor pagination built in. Default count of records is 50.
							Maximum count of records is 500 per call. Along with the records, this endpoint returns a
							next value and a topPageValue for ease of use in implementing pagination in applications. On
							initial calls (i.e. when no next value is passed in), a pageCount (total count of pages
							based on number of records and count) will also be returned. <br />
							<br />
						...
						<Table celled>
							...
							<Table.Body>
								...
								<Table.Row>
									...
									<Table.Cell>Starting date (must be older than EndDate)</Table.Cell>
								</Table.Row>
								<Table.Row>
					...
					<article className='right-article'>
						<img src={priceDateGif} alt='Example price by date endpoint' className='gif-examples' />
					</article>
				</section>
				...
				<section className='articles-examples' ref={pivotdocs}>
					<article className='left-article'>
						...
						<h3>Countries</h3>
						<p>
							Users have the ability to filter through the records based on the country. The countries are
							abbreviated below along with their full country names.
						</p>
						<Table celled>
							<Table.Header>
								<Table.Row>
					...
					<article className='right-article'>
						<img src={countryGif} alt='Example countries filter' className='gif-example-country' />
					</article>
				</section>
				<section className='articles-examples'>
					<article className='left-article'>
						<h3>Currency</h3>
						<p>
							The currency filter allows the user to convert search result retail and wholesale values to
							the selected currency. Currently, the returned values are not adjusted for inflation.
						</p>
						<Table celled>
							<Table.Header>
								<Table.Row>
					...
					<article className='right-article'>
						<img src={currenciesGif} alt='Example currency filter' className='gif-examples' />
					</article>
				</section>
				<section className='articles-examples'>
					<article className='left-article'>
						<h3>Markets</h3>
						<p>
							Markets range from all over Eastern Africa from Acura to Ziniya. There are currently over
							100 markets. Find multiple products in these markets as well as prices from specific items.
						</p>
					</article>
					<article className='right-article'>
						<img src={marketsGif} alt='Example markets filter' className='gif-examples' />
					</article>
				</section>
				<section className='articles-examples'>
					<article className='left-article'>
						<h3>Products</h3>
						<p>
							When using the products filters, user has the ability to search even more specific records.
							You must first understand that there is a hierarchy. The top is products category, second is
							products aggregators, and last is products. Using these filters in unison can bring up
							records of market prices based on a specific product.
						</p>
					</article>
					<article className='right-article'>
						<img src={productGif} alt='Example products filter' className='gif-examples' />
					</article>
				</section>
				<section className='articles-examples'>
					<article className='left-article'>
						<h3>Date Ranges</h3>
						<p>
							Market prices are always changing and being updated. We wanted the user to be able to search
							countries, markets, products and currency from different dates so the user can get the most
							up to date data for their use. The user can retrieve records back from 2013. With that
							ability to pick a start date and end date. We’ll generate every record in between for the
							user.
						</p>
					</article>
					<article className='right-article'>
						<img src={dateRangeGif} alt='Example date ranges filter' className='gif-examples' />
					</article>
				</section>
			</Container>
		</div>
